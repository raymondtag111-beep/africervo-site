...
exports.handler = async function (event) {
    ...
    try {
        ...
        // Les pages produits envoient le champ "imageUrl" (pas "icon") — on
        // accepte les deux pour rester compatible avec les 17 pages existantes.
        const productIcon = icon || imageUrl || 'icon-192.png';
        ...
        const adminUrl = `/admin.html?order=${orderId || ''}`;
        ...
        const message = {
            ...
            tokens: tokens,
            // PRIORITÉ HAUTE : sans ça, Android/Doze peut mettre le message
            // "normal" en attente plusieurs secondes (voire plus) avant de
            // réveiller le service worker — c'est la cause du délai de 13s.
            // "high" force une livraison quasi immédiate même app fermée.
            android: {
                priority: 'high'
            },
            webpush: {
                headers: { Urgency: 'high' },
                // Lien de secours avec l'ID de commande inclus, au cas où un
                // appareil affiche une notification par défaut au lieu de
                // passer par notre code (firebase-messaging-sw.js).
